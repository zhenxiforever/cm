...
import com.smile.wechat.util.WechatPropsUtil;
 ...
 */
public class WechatCoreUtil implements IWechatCore {
	...
	public WechatCoreUtil(WechatAccount wechatAccount) {
		super();
	...
	public void setWechatAccount(WechatAccount wechatAccount) {
		if(this.wechatAccount ==null ||
				!this.wechatAccount.getAppid().equals(wechatAccount.getAppid())){
			access_token = null;
		}
		this.wechatAccount = wechatAccount;
	}
	...
	public void getClient_credential() throws WechatException, JSONException {
		String url = type2uri(WechatPropsParam.WEIXIN_TOKEN_SUFFIX);
		url = String.format(url, wechatAccount.getAppid(), wechatAccount.getAppsecret());
	...
	public WechatUser getWXUserInfo(String openId) throws WechatException,
			JSONException {
		JSONObject json = doGetJson(WechatPropsParam.WEIXIN_USER_INFO_SUFFIX,
				access_token, openId);
	...
	public WechatUserOpenid getWXUserOpenId(String next_openid) throws WechatException,
		...
		JSONObject json = doGetJson(WechatPropsParam.WEIXIN_USER_GET_SUFFIX,
	...
	public void sendCustomTextMsg(String openId, String content)
		...
		json = doPostJson(WechatPropsParam.WEIXIN_MESSAGE_CUSTOM_SEND_SUFFIX, json);
		if (!JsonUtil.isRetSuccess(json)) {
	...
	public void createWXMenu(List<WechatMenu> menu_list) throws JSONException,
		...
		json = doPostJson(WechatPropsParam.WEIXIN_MENU_CREATE_SUFFIX, json);
		if (!JsonUtil.isRetSuccess(json)) {
			throw new WechatException(json.optString("errcode"),json.optString("errmsg"));
		}
	}
	...
	public List<WechatGroup> getWXGroups() throws WechatException, JSONException {
		JSONObject json = doGetJson(WechatPropsParam.WEIXIN_GROUPS_GET_SUFFIX,access_token);
	...
	public int createWXGroup(String name) throws WechatException, JSONException {
		...
		json = doPostJson(WechatPropsParam.WEIXIN_GROUPS_CREATE_SUFFIX, json);
		if (JsonUtil.isRetSuccess(json)) {
	...
	public int updateWXGroup(int id, String name) throws WechatException,
		...
		json = doPostJson(WechatPropsParam.WEIXIN_GROUPS_UPDATE_SUFFIX, json);
		if (JsonUtil.isRetSuccess(json)) {
			return id;
		} else {
			throw new WechatException("[errcode:" + json.optString("errcode")
	...
	public void updateWXGroupMember(String open_id, int to_group_id)
		...
		json = doPostJson(WechatPropsParam.WEIXIN_GROUPS_MEMBERS_UPDATE_SUFFIX, json);
		if (!JsonUtil.isRetSuccess(json)) {
			throw new WechatException("[errcode:" + json.optString("errcode")
					+ "]:" + json.optString("errmsg"));
		}
	...
	public String createQrcode(int action_type, int expire_seconds, int scene_id)
		...
		json = doPostJson(WechatPropsParam.WEIXIN_QRCODE_CREATE_SUFFIX, json);
		if (!JsonUtil.isRetSuccess(json)) {
			throw new WechatException("[errcode:" + json.optString("errcode")
					+ "]:" + json.optString("errmsg"));
		}
	...
	public TradeResultFromWechat sendTemplateMsg(String jsonstr) throws WechatException,
		...
		json = doPostJson(WechatPropsParam.WEIXIN_SEND_TEMPLATE_MSG_SUFFIX, json);
//		if (!JsonUtil.isRetSuccess(json)) {
//			throw new WechatException(json.optString("errcode"),json.optString("errmsg"));
//		}
	...
	public JSONObject uploadMedia(String mediaType, String mediaPath)
		...
		synchronized (lock) {
			...
			String url = WechatPropsUtil.getProperty(WechatPropsParam.WEIXIN_MEDIA_UPLOAD);
			...
			if (!checkAccess_token(json)) {// access_token过期
				getClient_credential();
				url = WechatPropsUtil.getProperty(WechatPropsParam.WEIXIN_MEDIA_UPLOAD);
				url = String.format(url, access_token, mediaType);
	...
	private String type2uri(String type) {
		return WechatPropsUtil.getProperty(WechatPropsParam.WEIXIN_SERVER_PREFIX)
				+ WechatPropsUtil.getProperty(type);
	}
